...
class Config:
    ...
    class Reward:
        ...
        class Scale:
            ...
            linvel_xy: float = -0.0#-2.0
            linvel_z: float = 0.0
            angvel: float = -0.0#-0.05
            orientation: float = -5.0#-5.0
            posture: float = -1.0#-2.0
            termination: float = -400.0
            torques: float = 0.0#-0.00025
            action_rate: float = -0.05#-0.05
            energy: float = 0.0#-0.0002
            feet_slip: float = 0.0
            knee_height: float = 0.0
            feet_deviation: float = -5.0#-5.0
            symmetry: float = -1.0#-1.0
            feet_air_time: float = -0.0#-10.0
            torque_rate: float = 0.0#-0.0000025
            joint_qvel_rate: float = -0.00001

        scale: Scale = default_field(Scale())
        tracking_sigma: float = 0.015
...
class Features:
    ...
    previous_actuator_force: Float[Array, "12"]
    joint_qvel: Float[Array, "12"]
    previous_joint_qvel: Float[Array, "12"]
    ...
    def step(
        ...
        return (
            Features(
                ...
                joint_qvel=joint_qvel,
                previous_joint_qvel=previous.joint_qvel,
                joint_angle_deltas=joint_angle_deltas,
                noisy_joint_angle_deltas=noisy_joint_angle_deltas
            ),
            done,
...
def reward(config: Config) -> Reward[Features]:
    def call(features: Features, done: Done) -> dict[str, Array]:
        def tracking_z():
            ...
            return jp.exp(-jp.sum(jp.square(error)) / sigma)
        ...
        def posture():
            ...
            in_threshold = jp.abs(error) < 0.05
            ...
            return jp.sum(jp.square(d)) + jp.sum(jp.abs(d))
        ...
        def feet_air_time():
            value = features.feet_air_time * ~features.feet_contacts

            return jp.sqrt(jp.sum(jp.square(value))) + jp.sum(jp.abs(value))
