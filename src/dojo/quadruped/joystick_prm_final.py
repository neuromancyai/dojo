...
class Config:
    ...
    class Reward:
        ...
        class Scale:
            ...
            energy: float = -0.002
            torques: float = -0.0009
            joint_qvel: float = -0.0045

            feet_slip: float = -0.1
            feet_clearance: float = -0.2
...
def reward(config: Config) -> Reward[Features]:
    ...
    def call(features: Features, done: Done) -> dict[str, Array]:
        ...
        def posture():
            weight = jp.array([1.0, 0.2, 0.0] * 4)

            return jp.sum(jp.square(features.joint_angle_deltas) * weight)
        ...
        def energy():
            e = jp.abs(features.joint_qvel) * jp.abs(features.actuator_force)
        ...
        def joint_qvel():
            return jp.sum(jp.square(features.joint_qvel))
